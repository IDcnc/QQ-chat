#!/usr/bin/env python3
# pack_ascii.py - Package QQBotManager (ASCII only, no Chinese characters)
...
def main():
    ...
    print("\n[2/6] Install qq-botpy v1.2.1...")
    if run_cmd("python -m pip install qq-botpy==1.2.1"):
        print("[INFO] qq-botpy installed successfully")
    else:
        print("[WARNING] qq-botpy installation failed, will use simulation mode")
    
    # Step 3: Install PyInstaller
    print("\n[3/6] Install PyInstaller...")
    if not run_cmd("python -m pip install pyinstaller"):
        print("[ERROR] PyInstaller installation failed!")
        input("Press Enter to exit...")
        return
    
    # Step 4: Kill old process
    print("\n[4/6] Kill old process...")
    run_cmd("taskkill /f /im QQBotManager.exe")
